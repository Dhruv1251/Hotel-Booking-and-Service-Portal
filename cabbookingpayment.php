...
<center><h4 style="font-size:25px;font-weight: bold; color:red;">Cab Booking Payment Gateway...</h4></center>
		</div>			
			<div>
...
<table id="dataTables" class="table table-striped table-bordered" cellspacing="0" width="100%">
	...
	<tbody>
	<?php
	$sql = "SELECT * FROM cab_booking LEFT JOIN vehicle_type ON cab_booking.vehicle_typeid=vehicle_type.vehicle_typeid LEFT JOIN customer ON cab_booking.customer_id=customer.customer_id WHERE cab_booking.cab_bookingid='$_GET[cab_bookingid]' AND cab_booking.status='Pending'";
	$qsql = mysqli_query($con,$sql);
	echo mysqli_error($con);
	while($rs = mysqli_fetch_array($qsql))
	{
...
<input type='hidden' name="cab_bookingid" id='cab_bookingid' value="<?php echo $_GET[cab_bookingid]; ?>" >
...
$('#btnpayment').bind('click', function(e) {
	...
	else
	{
		...
		var cab_bookingid = $('#cab_bookingid').val();
		...
		$.post("payment.php",
		{
			...
			'exp_date': exp_date,
			'totcost':totcost,
			'cab_bookingid':cab_bookingid,
			'room_booking_id':room_booking_id,
			'btncabbooking': "btncabbooking"
		},
		function(data, status){
			alert("Cab booking done successfully...");window.location='viewcabbooking.php';
		});
	}
});
</script>